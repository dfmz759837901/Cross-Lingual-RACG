...
DATASETS = ["mceval", "multilingual_humaneval", "mbxp", "total"]
RETRIEVAL_MODES = {"dense_CodeRankEmbed"}
SAMPLE_N = 1
            ...
            for poison in POISONS:
                ...
                for baseline_mode in baseline_modes:
                    ...
                    for result_lang_file in os.listdir(dir):
                        ...
                        if "baseline_fewshot" in result_file_path:
                            result_file_path = result_file_path.replace("dense_CodeRankEmbed", "dense_CodeRankEmbed_without_nl")
...
if __name__ == "__main__":
    ...
    for dataset in DATASETS:
        ...
        for retrieval_mode in RETRIEVAL_MODES:
            if dataset == "total":
                table_dict = {}
                for key in big_table[DATASETS[0]].keys():
                    assert key[1] in (
                        MODELS + ["mono-lingual-model", "multi-lingual-model"]
                    )
                    table_dict[key] = {
                        f"pass@{SAMPLE_N}": big_table[DATASETS[0]][key]["num"]
                        * big_table[DATASETS[0]][key][f"pass@{SAMPLE_N}"]
                        + big_table[DATASETS[1]][key]["num"]
                        * big_table[DATASETS[1]][key][f"pass@{SAMPLE_N}"]
                        + big_table[DATASETS[2]][key]["num"]
                        * big_table[DATASETS[2]][key][f"pass@{SAMPLE_N}"],
                        "num": big_table[DATASETS[0]][key]["num"]
                        + big_table[DATASETS[1]][key]["num"]
                        + big_table[DATASETS[2]][key]["num"],
                    }
                    table_dict[key][f"pass@{SAMPLE_N}"] /= table_dict[key]["num"]
                ...
                for baseline_mode in baseline_modes:
                    for language in LANGUAGES:
                        if (baseline_mode, MODELS[0], language) not in table_dict:
                            for model in MODELS:
                                table_dict[(baseline_mode, model, language)] = {
                                    f"pass@{SAMPLE_N}": 0,
                                    "num": 0,
                                }
                        ...
                        table_dict[(baseline_mode, "mono-lingual-model", language)] = {
                            ...
                            / 2,
                            "num": table_dict[(baseline_mode, "phi-1", language)][
                                "num"
                            ],
                        }
            ...
            mode = "w" if dataset == DATASETS[0] else "a"

            display(
                table_dict,
                sheet_name=dataset,
                save_path="table_G_exp_3.xlsx",
                mode=mode,
            )

        big_table[dataset] = table_dict
