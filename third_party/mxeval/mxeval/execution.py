...
def check_correctness_go_test(
    ...
    try:
        if not os.path.exists(os.path.join(cwd, "go.mod")):
            init_result = subprocess.run(
                "go mod init go-eval-test && go mod tidy",
                timeout=int(compile_timeout),
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
            ...
            compiled = init_result.returncode == 0
        else:
            compiled = True
        ...
        if compiled:
            ...
            passed, message, elapsed = False, init_result.stderr, None
    ...
    try:
        os.remove(path)
        os.remove(os.path.join(cwd, "go.mod"))
        os.remove(os.path.join(cwd, "go.sum"))
    except Exception as e:
        if verbose:
...
def check_correctness_go(
    ...
    if "func Test" in problem["test"] and "func main()" not in problem["test"]:
        return check_correctness_go_test(
            problem=problem,
            completion=completion,
            timeout=timeout,
            completion_id=completion_id,
...
def check_correctness_csharp(
    ...
    subprocess.check_call(
        f"unzip -q {template_cs_proj_zip} -d {cs_eval_dir}".split(),
...
def setup_base_path(current_dir, language_dirname, extension):
    with lock:
...
def check_correctness_helper(
    ...
    try:
        if compile_command_lambda is not None:
            ...
            compiled = (
                compile_result.returncode == 2
                if language == "typescript"
                else compile_result.returncode == 0
            )
        else:
            compiled = True
    ...
    if message is not None and "failed" in message:
        passed = False
    # get result
    return dict(
        task_id=problem["task_id"],
...
def check_correctness(
    ...
    def unsafe_execute():
        ...
        with create_tempdir():
            ...
            if "entry_point" in problem and problem["entry_point"] is not None:
