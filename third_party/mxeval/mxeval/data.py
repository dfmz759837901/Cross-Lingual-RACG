...
MULTILINGUAL_HUMANEVAL_METADATA = os.path.join(
    ROOT, "..", "data", "multilingual_humaneval", "metadata.json"
)
with open(MULTILINGUAL_HUMANEVAL_METADATA, "r", encoding="utf-8") as fr:
    MULTILINGUAL_HUMANEVAL_METADATA = json.load(fr)
HUMAN_EVAL_PYTHON = os.path.join(
    ROOT,
    "..",
    "data",
    "multilingual_humaneval",
    MULTILINGUAL_HUMANEVAL_METADATA["python"],
)
...
def get_metadata(dataset, metadata_type="problem"):
    ...
    typemap = {"problem": "metadata.json", "example": "metadata_examples.json"}
