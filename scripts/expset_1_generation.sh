...
for retrieval_mode in "${retrieval_modes[@]}"; do
    for generation_model_name_or_path in "${generation_model_paths[@]}"; do
        for generation_mode in "${generation_modes[@]}"; do
            ...
            log_filepath="logs/generation/${dataset}/${retrieval_mode}/${generation_mode}/log_${generation_model_name}.txt"
            ...
            echo "Generate Start: $generation_model_name_or_path with mode -g $generation_mode -r $retrieval_mode"
            ...
            echo "Generate Completed: $generation_model_name_or_path with mode -g $generation_mode -r $retrieval_mode"
        done
    done
done