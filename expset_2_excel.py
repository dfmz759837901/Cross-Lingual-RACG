...
def count(root_path: str, sample_n: int) -> Tuple[
    Dict[Tuple[str, str, str], float],
    Dict[Tuple[str, str, str], Dict[str, Union[float, int]]],
]:
    table = {}
    table_dict = {}
        ...
        for model_name in os.listdir(os.path.join(root_path, generation_mode)):
            for result_lang_file in os.listdir(
                ...
                if result_lang_file.endswith(".jsonl"):
                    ...
                    table_dict[(generation_mode, model_name, lang)] = {
...
if __name__ == "__main__":
    DATASETS = ["multilingual_humaneval", "mceval", "mbxp"]
    ...
    for DATASET in DATASETS:
        ...
        table, big_table[DATASET] = count(root_path, sample_n=SAMPLE_N)
        mode = "a" if DATASET != DATASETS[0] else "w"
        display(table, sheet_name=DATASET, save_path="table_G_exp_2.xlsx", mode=mode)

    total_table = {}
    generation_mode_list = ["baseline_fewshot"]  # , "RAG_cross_lang"]
    for language in LANGUAGES:
        generation_mode_list.append(f"RAG_lang_{language}")
    for model_name in MODELS:
        for generation_mode in generation_mode_list:
            for language in LANGUAGES:
                total_table[(generation_mode, model_name, language)] = 0.0
                total_num = 0
                for DATASET in DATASETS:
                    if (generation_mode, model_name, language) not in big_table[
                        DATASET
                    ]:
                        continue
                    eval_point = big_table[DATASET][
                        (generation_mode, model_name, language)
                    ]
                    total_table[(generation_mode, model_name, language)] += (
                        eval_point[f"pass@{SAMPLE_N}"] * eval_point["num"]
                    )
                    total_num += eval_point["num"]
                assert total_num > 0, f"{generation_mode}, {model_name}, {language}"
                total_table[(generation_mode, model_name, language)] /= total_num
    for generation_mode in generation_mode_list:
        for language in LANGUAGES:
            total_table[(generation_mode, "mono-lingual-model", language)] = (
                total_table[(generation_mode, "phi-1", language)]
                + total_table[(generation_mode, "phi-1_5", language)]
            ) / 2
            total_table[(generation_mode, "multi-lingual-model", language)] = (
                total_table[(generation_mode, "CodeLlama-7b-Instruct-hf", language)]
                + total_table[
                    (generation_mode, "deepseek-coder-6.7b-instruct", language)
                ]
                + total_table[(generation_mode, "Qwen2.5-Coder-7B-Instruct", language)]
            ) / 3
    display(total_table, sheet_name="total", save_path="table_G_exp_2.xlsx", mode="a")
